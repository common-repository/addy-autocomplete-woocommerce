...
if (in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')))) {
	...
	function register_addy_complete($checkout) {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Example event listener on shipping address field
            var addressField = document.querySelector('#shipping_address_1');
            if (addressField) {
                addressField.addEventListener('input', function(event) {
                    console.log('Address field value:', event.target.value);
                    // Initialize Addy here with event handling if needed
                });
            } else {
